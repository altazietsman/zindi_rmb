...
def load_models(model_name: str):
    ...
    if model == 'HoltWinters':
        return model_import(trend=model_params[1], seasonal=model_params[2], seasonal_periods=int(model_params[3]))

    elif model == 'AutoArima':
        return model_import()
    
    elif model == 'Prophet':
        return model_import(changepoint_range=int(model_params[1]), n_changepoints=int(model_params[2]), changepoint_prior_scale=float(model_params[3]))
